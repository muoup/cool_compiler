...
fairly straightforward. The d_tac_data file describes all of the TAC experssions that we might generate, and includes all of the
...
to generate TAC for each class and each method inside each class. To generate individual tac expression, we match each expression in the
...
opting instead to go directly to assembly code. Although may be against recommendations, the reason is that we felt it was
...
Before converting the TAC to assembly, we have almost 500 lines of assembly that is common to every .s file we generate. This code in
the a_builtins file is code that does not depend on the contents of whatever COOL program code is being generated for. It includes 
the primitives used in the IO and string methods and all runtime error messages and the assembly necessary to output them.
Although some parts of this file may not be required depending on the specific program being compiled (IO methods are unnecessary if no
method inherits IO), this structure allows a clear deliniation between the built-in funcions and the true compiled program, simplifying
...
we maintain a string map to let us get the string labels (made unique by the maintaining a simple counter increased every time a new 
string is generated) from strings in the program and a stack map, allowing us to access local variables and temporaries. Once we have this
...
complicated things like register allocaters. This also allows us to assume that no register ever contains a value we might like to retain,
...
The third test case address one our biggest flaws with our initial test suite, namely that most of our test cases were not large
enough to generate enough data to reveal flaws in our logic. The code in this test case is somewhat nonsensincal
if looked at closely, but it serves its purpose of touching almost all features of COOL, and thus almost all of
our code generator. Its voluminous output ensures that our code generator has to be pretty close to perfect to 
pass this test case. The final test case is a simple dispatch on void error, which checks our general error 
handling code and that we can detect dispatches on void. It was chosen becuase it was probably the most complicated
