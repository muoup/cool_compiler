...
in the end. The second change, or rather slate of changes, was to minimize the need for temporaries elsewhere.
The two main places this occurred was to pre-allocate space calls and dispatch arguments, which are all stored
...
instruction, and then compare it to zero, but can rather use a single conditional instruction if possible. With these,
...
instructions seem to have broken two and one test cases respectively. I (Zachary) wish I had more time to fix these,
...
them both in isolation to see if they are wholly unnecessary, like adding 0 or multiplying by 1, and also in pairs. 
Looking at instructions in pairs allows for much more helpful instruction removal, so this is where a majority
of the peephole optimizations are. We run these peephole optimizations on the assembly instructions until their
length doesn't change, since one optimization pass often opens up opportunities for another. For example, if we have
three adds of immediates to registers in a row, one pass will shorten this from three instructions to two, and 
a second will shorten it from two instructions to one. Once all these optimizations have been run to fixpoint, we
generate and output our assembly code.

We chose our benchmarks because they tend to generate code our compiler is good at optimizing. They are similar 
in that they both deal with manipulation of integer arithmetic. The first benchmark simply outputs the results
of arithmetic computations in a while loop, which our peephole optimizations are good at dealing with. The second
deals with if statements are integer arithmetic based on user inputs, since our compiler is 
able to successfully optimize them no matter what numbers are provided, although of course the optimization levels
vary based on how "optimizatable" the inputs are.
